...
6. Once the API is created, `copy to clipboard` and save it somewhere for later use
...
3. Give your project a name and click `Create repository from template` 
...
4. Change the placeholder ID [Google Sheet URL] and KEY [API Key] on lines 7 and 8 to the your copied information. Make sure to only edit the copy between the `' '` and leave no spaces.
...
6. Save your changes by clicking `Commit`
...
8. Repeat steps 1-3 for the `index.html` file on slides 5, 16, 18, and 29.

9. If using a custom domain name, repeat steps 1-3 for the `CNAME` file with your custom url (must purchase separately).
...
5. OPTIONAL: If you are using a custom domain, add it here and click `save`
